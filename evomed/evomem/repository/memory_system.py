...
class RepositorySystem:
    ...
    def analyze_contents_batch(self, contents: List[str], max_workers: int = 128) -> List[Dict]:
        """
        Batch analyze content using concurrent threads.
        Outputs progress and ETA to logs.
        """
        total = len(contents)
        results = [None] * total
        if total == 0:
        ...
        def _analyze_one(idx, text):
            try:
                r = self.analyze_content(text)   # reuse original analysis function
                results[idx] = r
            except Exception:
                results[idx] = {"keywords": [], "context": "General", "tags": []}

        start_time = time.time()
        completed = 0
        # Print progress every log_every items
        log_every = max(1, total // 20)

        logger.info(f"[LLM Analysis] Starting batch analysis of {total} items, max_workers={max_workers}")

        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(_analyze_one, i, contents[i])
                for i in range(total)
            ]

            for future in as_completed(futures):
                # Exception in thread will be exposed here
                _ = future.result()
                ...
                if completed % log_every == 0 or completed == total:
                    ...
                    logger.info(
                        f"[LLM Analysis Progress] {completed}/{total} "
                        f"({completed/total:.1%}), elapsed {elapsed/60:.1f} min, "
                        f"estimated remaining {eta/60:.1f} min"
                    )

        logger.info("[LLM Analysis] Batch analysis complete")
        return results
    ...
    def add_notes_batch(
        ...
        chroma_batch_size: int = 256,
        chroma_max_workers: int = 64,
    ) -> List[str]:
        ...
        if timestamps is not None and len(timestamps) != n:
            raise ValueError("timestamps length must match contents length")

        # 1) Batch LLM analysis (parallel)
        analyses = self.analyze_contents_batch(contents, max_workers=llm_max_workers)

        # 2) Construct MemoryNote objects (no evolution yet)
        notes: List[MemoryNote] = []
        for i, content in enumerate(contents):
            meta = extra_metadata[i] if extra_metadata else {}
            analysis = analyses[i] or {
                "keywords": [],
                "context": "General",
                "tags": [],
            }
            note = MemoryNote(
                ...
                bullet_id=meta.get("bullet_id"),
                retrieval_count=meta.get("retrieval_count"),
        ...
        # 3) Evolution: optional, parallel
        if enable_evolution:
            evo_log_every = 50  # Print progress every 50 memories
            evo_total = len(notes)
            evo_processed = 0

            def _evolve_one(idx_note):
                idx, note = idx_note
                _, evolved_note = self.process_memory(note)
                return idx, evolved_note

            with ThreadPoolExecutor(max_workers=128) as executor:
                futures = [
                    executor.submit(_evolve_one, (i, note))
                    for i, note in enumerate(notes)
                ]

                for future in as_completed(futures):
                    idx, evolved_note = future.result()
                    notes[idx] = evolved_note
                    evo_processed += 1

                    if evo_processed % evo_log_every == 0 or evo_processed == evo_total:
                        logger.info(
                            "[Memory Evolution] Progress %d/%d (%.1f%%), Example: id=%s bullet_id=%s tags=%s links=%s",
                            evo_processed,
                            evo_total,
                            evo_processed / evo_total * 100.0,
                            evolved_note.id,
                            getattr(evolved_note, "bullet_id", None),
                            evolved_note.tags,
                            evolved_note.links,
                        )

        # 4) Write to local memory and collect metadata for Chroma
        note_ids: List[str] = []
        documents: List[str] = []
        ...
        # 5) Batch write to Chroma
        self.retriever.add_documents_batch(
            documents=documents,
            metadatas=metadatas,
            ids=note_ids,
            batch_size=chroma_batch_size,
            max_workers=chroma_max_workers,
        )
    ...
    def consolidate_memories(self):
        """Consolidate memories: rebuild retriever index with all documents"""
        ...
        # Batch write with 64-thread embedding
        self.retriever.add_documents_batch(
            documents=documents,
            metadatas=metadatas,
            ids=ids,
    ...
    def process_memory(self, note: MemoryNote) -> Tuple[bool, MemoryNote]:
        ...
        try:
            ...
            try:
                ...
                if should_evolve:
                    # Maintain only 'strengthen' action, ignore others
                    raw_actions = response_json.get("actions", [])
                    actions = [a for a in raw_actions if a == "strengthen"]
