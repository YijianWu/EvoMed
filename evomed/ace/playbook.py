...
class FixedModules:
    ...
    def to_vector_text(self) -> str:
        ...
        if self.contextual_states.scenario:
            parts.append(f"情境: {self.contextual_states.scenario}")
        if self.contextual_states.chief_complaint:
            parts.append(f"主诉: {self.contextual_states.chief_complaint}")
        if self.contextual_states.core_symptoms:
            parts.append(f"核心症状: {self.contextual_states.core_symptoms}")
        if self.decision_behaviors.diagnostic_path:
            parts.append(f"诊断路径: {self.decision_behaviors.diagnostic_path}")
...
class ModularBullet:
    ...
    def as_prompt_text(self) -> str:
        ...
        if self.fixed_modules.contextual_states.scenario:
            lines.append(f"  情境: {self.fixed_modules.contextual_states.scenario}")
        if self.fixed_modules.contextual_states.chief_complaint:
            lines.append(f"  主诉: {self.fixed_modules.contextual_states.chief_complaint}")
        if self.fixed_modules.contextual_states.core_symptoms:
            lines.append(f"  核心症状: {self.fixed_modules.contextual_states.core_symptoms}")
        if self.fixed_modules.decision_behaviors.diagnostic_path:
            lines.append(f"  诊断路径: {self.fixed_modules.decision_behaviors.diagnostic_path}")
        if self.mutable_modules.uncertainty.primary_uncertainty:
            lines.append(f"  不确定性: {self.mutable_modules.uncertainty.primary_uncertainty}")
...
class ModularPlaybook:
    """模块化经验库：支持固定模块（向量检索）和可迭代模块（反思修改）"""
