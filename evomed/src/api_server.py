"""
HTTP API 服务器
基于 FastAPI 实现的医疗Diagnosis服务 HTTP 接口
...
class PatientData(BaseModel):
    """Patient病历数据"""
    patientName: Optional[str] = Field(None, description="Patient姓名")
    patientGender: Optional[str] = Field(None, description="PatientGender (F/M/女/男)")
    patientAge: Optional[int] = Field(None, description="PatientAge")
...
class ECGData(BaseModel):
    ...
    pred: Optional[bool] = Field(False, description="预测结果: False=正常, True=异常")
    conf: Optional[float] = Field(0.5, description="置信度")
    topk: Optional[List] = []
...
class DiagnosisRequest(BaseModel):
    """Diagnosis请求"""
    patient: PatientData = Field(..., description="Patient病历数据")
    ...
    max_experts: Optional[int] = Field(5, description="最大激活Expert数量", ge=1, le=10)
...
app = FastAPI(
    title="多专科医疗Diagnosis服务 API",
    description="基于可演化Expert池(EEP)的医疗Diagnosis辅助系统",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def startup_event():
    ...
    print("="*60)
    print("正在启动医疗Diagnosis服务...")
    print("="*60)
    try:
        diagnosis_api = DiagnosisAPI()
        print("✅ Diagnosis服务初始化成功")
    except Exception as e:
        print(f"❌ Diagnosis服务初始化失败: {e}")
        raise
...
async def health_check():
    ...
    if diagnosis_api is None:
        raise HTTPException(status_code=503, detail="Diagnosis服务未初始化")
...
@app.post("/api/v1/diagnose", tags=["Diagnosis服务"])
async def diagnose(request: DiagnosisRequest):
    """
    执行医疗Diagnosis
    
    输入：
    - patient: Patient病历数据（必需）
    - bowel_sound: 肠鸣音检测结果（可选）
    - ecg: ECG检测结果（可选）
    - max_experts: 最大激活Expert数量（可选，默认5）
    - for_doctor: 是否返回医生端格式（可选，默认False）
    
    输出：
    - Patient端格式：包含Diagnosis结果、Diagnosis依据、鉴别Diagnosis、建议检查
    ...
    try:
        ...
        print(f"Diagnosis过程出错: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Diagnosis失败: {str(e)}")


@app.post("/api/v1/diagnose/patient", tags=["Diagnosis服务"])
async def diagnose_patient_view(request: DiagnosisRequest):
    """
    Patient端Diagnosis接口（返回Patient可见信息）
    """
    request.for_doctor = False
    return await diagnose(request)


@app.post("/api/v1/diagnose/doctor", tags=["Diagnosis服务"])
async def diagnose_doctor_view(request: DiagnosisRequest):
    """
    医生端Diagnosis接口（返回完整信息，包括风险评估）
    """
    request.for_doctor = True
    return await diagnose(request)


@app.get("/api/v1/experts", tags=["Expert池管理"])
...
@app.get("/api/v1/specialties", tags=["Expert池管理"])
...
def main():
    parser = argparse.ArgumentParser(description="多专科医疗Diagnosis服务 HTTP API")
    ...
    print("="*60)
    print(f"启动医疗Diagnosis服务 HTTP API")
