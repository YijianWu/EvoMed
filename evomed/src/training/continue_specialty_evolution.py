"""
继续运行剩余Department的Expert池训练

从中断处继续，只运行尚未完成的Department
"""
            ...
            try:
                with open(intermediate_file, 'r', encoding='utf-8') as f:
                    ...
                    if len(data) > 0:
                        ...
                        print(f"  ✅ {specialty}: 已完成 ({len(data)} 位Expert)")
            except:
...
def main():
    ...
    print(f"\n📊 进度统计:")
    print(f"  - 已完成: {len(completed)} 个Department")
    print(f"  - 待处理: {len(remaining)} 个Department")
...
def merge_all_results():
    """合并所有Department的Expert池"""
    all_experts = []
    
    print("\n📂 读取所有Department的Expert池...")
    for specialty in SPECIALTIES:
        intermediate_file = f"specialty_pool_{specialty}.json"
        if os.path.exists(intermediate_file):
            try:
                with open(intermediate_file, 'r', encoding='utf-8') as f:
                    experts = json.load(f)
                    all_experts.extend(experts)
                    print(f"  ✅ {specialty}: {len(experts)} 位Expert")
            except Exception as e:
