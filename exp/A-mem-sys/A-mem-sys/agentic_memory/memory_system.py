...
class AgenticMemorySystem:
    ...
    def analyze_contents_batch(self, contents: List[str], max_workers: int = 128) -> List[Dict]:
        """
        批量分析内容，max_workers 并发线程，每条仍调用一次 LLM，但通过并发加速。
        会在日志中输出进度和预计剩余时间。
        """
        total = len(contents)
        results = [None] * total
        ...
        def _analyze_one(idx, text):
            try:
                r = self.analyze_content(text)   # <-- 复用你原来的分析函数
                results[idx] = r
            except Exception:
        ...
        completed = 0
        # 每处理 log_every 条打印一次进度，大概 20 个 step
        log_every = max(1, total // 20)

        logger.info(f"[LLM分析] 开始批量分析 {total} 条内容，max_workers={max_workers}")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                if completed % log_every == 0 or completed == total:
                    ...
                    logger.info(
                        f"[LLM分析进度] {completed}/{total} "
                        f"({completed/total:.1%})，已用 {elapsed/60:.1f} 分钟，"
                        f"预计剩余 {eta/60:.1f} 分钟"
                    )

        logger.info("[LLM分析] 批量分析完成")
        return results
    ...
    def add_notes_batch(
        ...
        # 3）演化：可选，并行（128 线程）
        if enable_evolution:
            evo_log_every = 50  # 每处理多少条记忆打印一次演化进度和示例
            evo_total = len(notes)
            evo_processed = 0
            ...
            with ThreadPoolExecutor(max_workers=128) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    # 每处理 evo_log_every 条（或最后一条）打印一次进度 + 一个演化示例
                    if evo_processed % evo_log_every == 0 or evo_processed == evo_total:
                        logger.info(
                            "[记忆演化] 进度 %d/%d (%.1f%%)，示例：id=%s bullet_id=%s tags=%s links=%s",
                            evo_processed,
                            evo_total,
    ...
    def consolidate_memories(self):
        """Consolidate memories: update retriever with new documents（批量重建）"""
    ...
    def process_memory(self, note: MemoryNote) -> Tuple[bool, MemoryNote]:
        ...
        try:
            ...
            try:
                ...
                if should_evolve:
                    # 只保留 strengthen 行为，显式忽略 update_neighbor 等其他动作
