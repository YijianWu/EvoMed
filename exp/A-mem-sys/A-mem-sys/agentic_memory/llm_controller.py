...
class OpenAIController(BaseLLMController):
    def __init__(self, model: str = "gpt-4", api_key: Optional[str] = None):
        try:
            ...
            if not api_key:
                raise ValueError("API key not provided. Set YUNWU_API_KEY or OPENAI_API_KEY.")

            base_url_env = os.getenv("YUNWU_BASE_URL") or os.getenv("OPENAI_BASE_URL") or "https://api.openai.com/v1"
            self.client = OpenAI(api_key=api_key, base_url=base_url_env)
        except ImportError:
