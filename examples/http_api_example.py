...
def check_health():
    ...
    print(f"Expert数量: {result.get('expert_count', 'N/A')}")
    print()
...
def list_experts():
    ...
    print(f"总Expert数: {result['total_experts']}")
...
def list_specialties():
    ...
    for specialty, info in result['specialties'].items():
        print(f"  {specialty}: {info['expert_count']}位Expert, "
              f"平均fitness={info['avg_fitness']:.3f}")
    print()


def diagnose_patient_basic():
    """基础Diagnosis示例（Patient端）"""
    print("="*60)
    print("4. 基础Diagnosis示例（Patient端）")
    print("="*60)
    ...
    patient_data = {
        "patient": {
            ...
            "presentIllness": """Patient2天前无明显诱因出现上腹部疼痛，呈阵发性绞痛，
...
疼痛与进食油腻食物有关。无发热，无黄疸，大小便正常。""",
    ...
    if response.status_code == 200:
        ...
        print(f"❌ Diagnosis失败: {response.status_code}")
        print(response.text)
...
def diagnose_doctor_full():
    """完整Diagnosis示例（医生端，含风险评估）"""
    print("="*60)
    print("5. 完整Diagnosis示例（医生端 + 风险评估）")
    print("="*60)
    ...
    patient_data = {
        "patient": {
            ...
            "presentIllness": """Patient3天前无明显诱因出现右上腹痛，呈持续性胀痛，
...
def save_result_to_file():
    ...
    if response.status_code == 200:
        ...
        print(f"✅ Diagnosis结果已保存到: {output_file}")
    else:
        print(f"❌ Diagnosis失败: {response.status_code}")
