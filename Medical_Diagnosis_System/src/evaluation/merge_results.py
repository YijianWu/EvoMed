...
for f in json_files:
    ...
    row = {
        ...
        '激活专家': ', '.join(routing.get('selected_experts', [])),
        'Step1_路由建议': step1.get('output', ''),
        '各专家诊断意见': '\n\n'.join(expert_diag_list),
        'Step4_综合诊断': step4.get('output', ''),
        '新专家生成': '是' if evolution.get('new_expert_generated') else '否',
        '新专家名称': evolution.get('new_expert_name', ''),
