...
def run_batch_diagnosis(
    ...
    Args:
        ...
        top_k: 激活专家数量
        enable_rag: 启用RAG
        enable_experience: 启用经验库
        enable_case: 启用病例库
        start_index: 起始病例索引
        max_cases: 最大处理病例数（None表示全部）
    """
    print("=" * 70)
    print("批量多学科诊断系统")
    print("=" * 70)
    ...
    print(f"\n处理范围: 病例 {start_index + 1} 到 {end_index}")
    print(f"激活模式: {activation_mode} | 专家数量: {top_k}")
    print("=" * 70)
    ...
    for idx in range(start_index, end_index):
        ...
        try:
            ...
            if not patient:
                print(f"⚠️ 病例 {case_num} 解析失败，跳过")
                summary["failed"] += 1
                continue
            
            print(f"患者: {patient.gender}, {patient.age}岁")
            print(f"主诉: {patient.chief_complaint[:50]}..." if len(patient.chief_complaint) > 50 else f"主诉: {patient.chief_complaint}")
            ...
            print(f"\n✅ 病例 {case_num} 完成，耗时 {case_duration:.1f}s，结果保存至: {output_file}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="批量多学科诊断")
    ...
    parser.add_argument("--max", type=int, default=None, help="最大病例数")
