...
class DiagnosticPipeline:
    ...
    def __init__(
        ...
        expert_pool_path: str = "expert_pool.json",
        evolved_pool_path: str = "optimized_expert_pool.json",
    ...
    def _activate_experts_evolved(
        ...
        Args:
            ...
            selection_strategy: 选择策略
                - 'top_fitness': 按适应度选择前K个
                - 'diversity': 多样性选择（确保不同风格）
                - 'hybrid': 混合策略（适应度 + 专科匹配）
        ...
        if selection_strategy == "top_fitness":
            # 按适应度排序，选择前K个
            ...
            # 混合策略：根据专科和适应度综合选择
            # 为每个基础专家选择最匹配的进化Prompt
            sorted_pool = sorted(self.evolved_pool, key=lambda x: x.get('fitness', 0), reverse=True)
            
            for i, expert in enumerate(base_experts):
                # 优先使用高适应度的Prompt，但确保多样性
                # 使用模运算确保不同专家使用不同的进化Prompt
                idx = i % len(sorted_pool)
                evolved_prompts.append(sorted_pool[idx])
        ...
        for i, (expert, prompt_config) in enumerate(zip(base_experts, evolved_prompts)):
            if prompt_config:
                ...
                print(f"  [{i+1}] {expert['name']} <- {prompt_id} (Fitness: {fitness:.4f if isinstance(fitness, float) else fitness})")
            else:
...
def main(
    ...
    if activation_mode == "train_ga":
        ...
        # 保存最优专家池
        pool_file = "optimized_expert_pool.json"
...
if __name__ == "__main__":
    ...
    # 进化专家池参数
    parser.add_argument("--evolved_pool", type=str, default="optimized_expert_pool.json",
