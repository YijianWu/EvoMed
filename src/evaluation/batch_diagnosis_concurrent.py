...
支持多线程并发运行Diagnosis，输出Excel汇总结果
"""
...
def process_single_case(
    ...
    try:
        ...
        safe_print(f"[{case_num}] 开始Diagnosis: {patient.gender}, {patient.age}岁")
...
def run_concurrent_diagnosis(
    ...
    print(f"将处理 {total_cases} 个Case（共 {len(df)} 个）")
    ...
    print(f"\n并发数: {max_workers} | 激活模式: {activation_mode} | Expert数量: {top_k}")
    print("=" * 70)
    ...
    for r in results:
        ...
        if r["summary"]:
            row["激活Expert"] = r["summary"].get("activated_experts", "")
            row["综合Diagnosis"] = r["summary"].get("final_diagnosis", "")
    ...
    print(f"平均每Case: {total_duration/total_cases:.1f}s")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="并发批量多学科Diagnosis")
    ...
    parser.add_argument("--max", type=int, default=20, help="最大Case数")
